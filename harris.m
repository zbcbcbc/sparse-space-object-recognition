...
function [x,cim] = harris(im, coe, disp)

    sigma = coe.sigma;
    thresh = coe.threshold;
    radius = coe.radius;

    
    if disp == true
        figure(4); clf; imagesc(im); colormap(gray); title('corners detected');axis image;
    end  

    dx = [-1 0 1; -1 0 1; -1 0 1]; % Derivative masks
    dy = dx';
    
    Ix = conv2(double(im), double(dx), 'same');    % Image derivatives
    Iy = conv2(double(im), double(dy), 'same');    
...
%   cim = (Ix2.*Iy2 - Ixy.^2) - k*(Ix2 + Iy2).^2; 
    ...
    if disp == true
        figure(4); hold on; plot(c, r, 'ys'); hold off
    end
